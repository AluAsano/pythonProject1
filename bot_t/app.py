...
def stop(message: telebot.types.Message):
    text = f'До встречи, {message.chat.username}!'
    bot.reply_to(message, text)
...
def get_price(message: telebot.types.Message):
    try:
        ...
        if len(values) != 3:
            raise APIException("Введены некорректный параметры")
        ...
        text = f'Цена {amount} {base} в {quote} - {float(total_quote) * float(amount):.2f}'
